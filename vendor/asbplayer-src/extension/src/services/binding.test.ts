...
describe('Binding playback mode integration', () => {
    ...
    const requestPlaybackState = (binding: Binding) => {
        const responses: any[] = [];
        const request = {
            sender: 'asbplayer-extension-to-video',
            src: binding.registeredVideoSrc,
    ...
    it('answers request-playback-state with the position in milliseconds and the playing flag', async () => {
        const video = createVideo();
        const binding = new Binding(video, bindingOptions(false, false));
        binding.bind();
        await jest.advanceTimersByTimeAsync(0);
        ...
        // The video's own clock is in seconds - the response must be in milliseconds.
        expect(video.currentTime).toBe(1.5);
        expect(requestPlaybackState(binding)).toEqual([{ timestampMs: 1500, playing: true }]);

        Object.defineProperty(video, 'paused', { configurable: true, value: true });

        expect(requestPlaybackState(binding)).toEqual([{ timestampMs: 1500, playing: false }]);
        binding.unbind();
    });
