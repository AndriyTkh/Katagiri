...
import type {
    ...
    RequestPlaybackStateMessage,
    RequestPlaybackStateResponse,
...
const requestPlaybackStateFromVideoElement = async (
    tabId: number,
    src: string
): Promise<RequestPlaybackStateResponse | undefined> => {
    const requestPlaybackStateCommand: ExtensionToVideoCommand<RequestPlaybackStateMessage> = {
        sender: 'asbplayer-extension-to-video',
        src,
        message: { command: 'request-playback-state' },
    };

    try {
        const response: RequestPlaybackStateResponse | undefined = await browser.tabs.sendMessage(
            tabId,
            requestPlaybackStateCommand
        );
        return response;
    } catch {
        // Targeting a non-active/discarded tab can fail
...
export const bindWebSocketClient = async (settings: SettingsProvider, tabRegistry: TabRegistry) => {
    ...
    client.onGetPlaybackState = async (mediaId: string | undefined): Promise<MediaPlaybackState | null> => {
        const [target] = await resolveMediaTargets(tabRegistry, mediaId);

        // Local asbplayer app media does not expose its playback position to the extension
        if (target === undefined || !isVideoElementTarget(target)) {
            return null;
        }

        const { videoElement } = target;
        const playbackState = await requestPlaybackStateFromVideoElement(videoElement.id, videoElement.src);
        ...
        return {
            mediaId: streamingMediaId(videoElement.id, videoElement.src),
            timestampMs: Math.round(playbackState.timestampMs),
