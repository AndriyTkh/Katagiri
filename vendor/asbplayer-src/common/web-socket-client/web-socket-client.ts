...
export interface GetPlaybackStateCommand {
...
interface GetPlaybackStateResponseBody {
...
export class WebSocketClient {
    ...
    onGetPlaybackState?: (mediaId: string | undefined) => Promise<MediaPlaybackState | null>;
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.onmessage = async (event) => {
                if (event.data === 'PONG') {
                    ...
                    } else if (payload.command === 'get-playback-state') {
                        if (this.onGetPlaybackState !== undefined) {
                            const messageId = payload.messageId;
                            const playbackState = await this.onGetPlaybackState(payload.body?.mediaId);
                            const response: Response<GetPlaybackStateResponseBody> = {
                                command: 'response',
                                messageId,
                                body: { playbackState: playbackState ?? null },
                            };
                            this._socket?.send(JSON.stringify(response));
                        }
                    }
    ...
    unbind() {
        ...
        this.onGetPlaybackState = undefined;
    }
}
