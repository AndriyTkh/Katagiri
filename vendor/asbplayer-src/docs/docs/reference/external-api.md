...
> This command is only available when extension v1.21.0+ (unreleased) is installed

Returns the current playback position of a piece of streaming media. By default it targets the active tab's media; pass a `mediaId` from [`get-bound-media`](#get-bound-media) to target specific media.
`playbackState` is `null` when no matching media is found, when the media is local, or when the media's tab does not respond.

#### Request

```javascript
{
    "command": "get-playback-state",
    // Message ID to correlate with asbplayer's response
    "messageId": "5c3f7e91-0b2d-4a68-9e51-7d4a6b8c0f23",
    "body": {
        // Optional media id from `get-bound-media`. Defaults to the active tab when omitted.
        "mediaId": "a1b2c3d4e5f6"
    }
...
```javascript
{
    ...
    "body": {
        "playbackState": {
            // Identifier for the media, matching the ids returned by `get-bound-media`
            "mediaId": "a1b2c3d4e5f6",
...
- `GET asbplayer/playback-state` (optional `?mediaId=...`) ([script](https://github.com/asbplayer/asbplayer/blob/main/scripts/web-socket-server/cli/playback-state))
