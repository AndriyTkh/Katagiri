...
func (forwarder forwarder) handleAsbplayerPlaybackStateRequest(c echo.Context) error {
	body := map[string]interface{}{}
	if mediaId := c.QueryParam("mediaId"); mediaId != "" {
		body["mediaId"] = mediaId
	}
	command := clientCommand{Command: "get-playback-state", MessageId: uuid.NewString(), Body: body}
	responseChannel := make(chan clientResponse)
...
func main() {
	...
	fmt.Printf("Started with config:\n\n\tHOST=%v\n\tPORT=%v\n\tANKI_CONNECT_URL=%v\n\tPOST_MINE_ACTION=%v\n\tINTERCEPT_FIELD=%v\n\tINTERCEPT_VALUE=%v\n",
		host, port, ankiConnectUrl, postMineAction, interceptField, interceptValue)
	...
	e.GET("/asbplayer/playback-state", forwarder.handleAsbplayerPlaybackStateRequest)
