...
- [ ] T010 [P] [US3] mokuro channel: page-change bridge on the port pinned in T004, shared secret from config `_SECRET_KEYS` + Origin validation; volume-data.json poller fallback; .mokuro text layer. Tests bind port 0 / mock the bridge — never the pinned port. Worktree `wt/e-mokuro`. Write: NEW src/katagiri/media_mokuro.py, NEW tests/test_media_mokuro.py. Read: media_channel.py interface, config.py phase-E keys (secret + port), research.md mokuro notes, T004 hardening tests. [was: kata-e3]
- [ ] T011 [P] [US5] Lyrics (mpv WATCH pipeline per FR-005): .lrc/.ass through the subtitle pipeline; lyric lines minable with source refs. Worktree `wt/e-lyrics`. Write: NEW src/katagiri/media_lyrics.py, NEW tests/test_lyrics.py. Read: media_channel.py + media_mpv.py subtitle pipeline, spec.md FR-005. [was: kata-e5]
- [ ] T012 Registration (serial, master, after lane merges in the order above): additive ToolSpec batch for all TG-E4 tools + adapter blocks + smoke tests. Write: tool_registry.py, mcp_server.py, tests/test_mcp_tools.py. Read: seam layout, new module APIs. [was: kata-e2..e5]
