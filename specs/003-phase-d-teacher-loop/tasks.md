...
- [x] T010 [US2] Lesson memory (unresolved[], next_step, revisit_after — schedules topics, never items) + Today.md section renderer plugged into the `SECTIONS` seam. Write: NEW src/katagiri/lesson_memory.py, src/katagiri/today_export.py (append one `@section` builder + one tuple entry only). Read: today_export.py:700–720 (SECTIONS seam docs), docs/db-schema.md, session_tools.py API. [was: kata-d4]
...
- [x] T012 [P] [US2] Unit tests for lesson memory incl. Today.md section rendering. Write: NEW tests/test_lesson_memory.py. Read: fixture recipe, lesson_memory.py API, tests/test_today.py (section-test pattern). [was: kata-d4]
