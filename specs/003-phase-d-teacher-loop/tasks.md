...
- [ ] T005 [in-progress: lane-d-session] [US1] start_session (returns exactly one prescribed action), log_lesson, lessons(topic?, unresolved_only). Lane `wt/d-session`. Write: NEW src/katagiri/session_tools.py. Read: docs/db-schema.md (event/observation/lesson DDL), src/katagiri/events.py (`append_event` — the event-write path), envelope.py API (from T004). [was: kata-d3]
- [ ] T006 [in-progress: lane-d-session] [US1] log_observations with enforced mandatory fields (unassisted, coverage band, rubric_version), log_error, add_vocab, triage_inbox. Write: src/katagiri/session_tools.py (serial after T005, same file). Read: T005 output, docs/db-schema.md. [was: kata-d3]
